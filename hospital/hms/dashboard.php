...
function getDoctorSchedule(mysqli $con, $doctorId) {
    ...
    if ($hasWorkingDays && $hasStart && $hasEnd) {
        ...
        if($stmt){
            ...
            $stmt->close();
            if ($r) {
                $st = norm_time($r['startTime']); $en = norm_time($r['endTime']);
                $map = [
                    'الاحد'=>0,'الأحد'=>0,'احد'=>0,'sun'=>0,
                    'الاثنين'=>1,'الإثنين'=>1,'اثنين'=>1,'mon'=>1,
                    'الثلاثاء'=>2,'ثلاثاء'=>2,'tue'=>2,
                    'الاربعاء'=>3,'الأربعاء'=>3,'اربعاء'=>3,'wed'=>3,
                    'الخميس'=>4,'thu'=>4,'الجمعة'=>5,'fri'=>5,'السبت'=>6,'sat'=>6
                ];
                $parts = array_filter(array_map('trim', preg_split('/[,|\s]+/u', mb_strtolower($r['days'],'UTF-8'))));
                foreach ($parts as $p) {
                    if ($p==='') continue;
                    if (is_numeric($p)) {
                        $d = (int)$p; if ($d>=1 && $d<=7) $d=$d%7;
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['book_submit'])) {
    ...
    if (!$doc) { $_SESSION['msg']='الطبيب غير موجود.'; header("Location: $back"); exit; }
    ...
    if (empty($sched['days']) || empty($sched['days'][$dow])) {
        $_SESSION['msg'] = 'اليوم المختار خارج دوام الطبيب.'; header("Location: $back"); exit;
    }
    // تأكد أن الوقت ضمن أي فترة من فترات نفس اليوم
    $okTime=false;
    foreach ($sched['days'][$dow] as $p){
        if ($time >= $p['start'] && $time < $p['end']) { $okTime=true; break; }
    }
    if(!$okTime){
        $_SESSION['msg'] = 'الوقت المختار خارج دوام الطبيب.'; header("Location: $back"); exit;
    }
    ...
    if ($stmt = $con->prepare("SELECT COUNT(*) FROM appointment WHERE doctorId=? AND appointmentDate=? AND appointmentTime=? AND userStatus=1 AND doctorStatus=1")) {
        ...
        $stmt->execute(); $stmt->bind_result($busy); $stmt->fetch(); $stmt->close();
    }
    if ($busy>0) { $_SESSION['msg']='الوقت محجوز بالفعل، اختر وقتًا آخر.'; header("Location: $back"); exit; }
...
if (isset($_GET['ajax'])) {
    ...
    if ($action==='specs') {
        ...
        echo json_encode(['ok'=>true,'data'=>$arr], JSON_UNESCAPED_UNICODE); exit;
    }
    ...
    if ($action==='doctors_by_spec') {
        ...
        echo json_encode(['ok'=>true,'data'=>$out], JSON_UNESCAPED_UNICODE); exit;
    }
    ...
    if ($action==='doctor_info') {
        ...
        if(!$stmt){ http_response_code(500); echo json_encode(['ok'=>false,'msg'=>'DB Error'], JSON_UNESCAPED_UNICODE); exit; }
        $stmt->bind_param("i",$id); $stmt->execute();
        $doc = $stmt->get_result()->fetch_assoc(); $stmt->close();
        if (!$doc) { http_response_code(404); echo json_encode(['ok'=>false,'msg'=>'الطبيب غير موجود'], JSON_UNESCAPED_UNICODE); exit; }
        $sched = getDoctorSchedule($con, $id);
        echo json_encode(['ok'=>true,'data'=>$doc,'schedule'=>$sched], JSON_UNESCAPED_UNICODE); exit;
    }

    if ($action==='slots') {
        $id = (int)($_GET['id'] ?? 0);
        $date = trim($_GET['date'] ?? '');
        if ($id<=0 || !$date) { http_response_code(400); echo json_encode(['ok'=>false,'msg'=>'بيانات غير مكتملة'], JSON_UNESCAPED_UNICODE); exit; }
        ...
        if (!empty($sched['days'][$dow])) {
            ...
            foreach ($sched['days'][$dow] as $p){
                $start = $p['start']; $end = $p['end']; $stepMin = (int)$p['slot'] ?: 30;
                $t = strtotime($start);
                $endTs = strtotime($end);
                while ($t < $endTs) {
                    $slots[] = date('H:i', $t);
        ...
        echo json_encode(['ok'=>true,'data'=>$slots,'hasSchedule'=>!empty($sched['days'])], JSON_UNESCAPED_UNICODE); exit;
    }

    echo json_encode(['ok'=>false,'msg'=>'طلب غير معروف'], JSON_UNESCAPED_UNICODE); exit;
}
...
<head>
  ...
  <style>
    :root{--primary:#3498db;--secondary:#1a2530}
    body{font-family:'Tajawal',sans-serif;background:#f0f5f9;color:#333;padding-top:60px;padding-bottom:76px}
    .layout{display:flex;gap:20px}
    .sidebar{width:260px;background:#fff;border-radius:15px;box-shadow:0 5px 15px rgba(0,0,0,.05);padding:20px;height:100%}
    .patient-card{display:flex;align-items:center;gap:12px;padding:14px;border:1px dashed #e5eef6;border-radius:12px;background:#fbfdff;margin-bottom:16px}
    .patient-avatar{width:44px;height:44px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:linear-gradient(90deg,var(--primary),#4aa8e0);color:#fff;font-weight:700}
    .menu{list-style:none;padding:0;margin:0}
    .menu li{margin-bottom:8px}
    .menu a{display:flex;align-items:center;gap:10px;text-decoration:none;color:#22313f;padding:10px 12px;border-radius:10px}
    ...
    .app-header{background:linear-gradient(90deg,#3498db,#4aa8e0);color:#fff;padding:22px 26px;border-radius:10px;margin-bottom:20px;box-shadow:0 5px 15px rgba(0,0,0,.1)}
    .main{flex:1}
    ...
    .disease-card{background:#fff;border:1px solid #e9f1f7;border-radius:14px;padding:16px;text-align:center;cursor:pointer;transition:.2s;box-shadow:0 2px 8px rgba(0,0,0,.02)}
    .disease-card:hover{transform:translateY(-4px);box-shadow:0 8px 18px rgba(0,0,0,.08)}
    .disease-card i{font-size:1.8rem;color:#3498db;margin-bottom:8px}
    .appt-card{display:flex;gap:14px;align-items:center}
    .appt-icon{width:48px;height:48px;border-radius:10px;background:#eef7ff;display:flex;align-items:center;justify-content:center}

    /* نافذة الحجز */
    .rt-modal{position:fixed;inset:0;display:none;z-index:200000}
    .rt-modal.open{display:block}
    .rt-backdrop{position:absolute;inset:0;background:rgba(0,0,0,.55)}
    .rt-dialog{position:relative;margin:6vh auto;width:min(92vw,900px);max-width:900px;background:#fff;border-radius:14px;box-shadow:0 12px 40px rgba(0,0,0,.25);overflow:hidden;direction:rtl;text-align:right;max-height:88vh;display:flex;flex-direction:column}
    .rt-header,.rt-footer{padding:12px 16px;border-bottom:1px solid #eee}
    .rt-footer{border-bottom:0;border-top:1px solid #eee;display:flex;gap:8px}
    .rt-title{margin:0;font-weight:700}
    .rt-close{position:absolute;left:12px;top:8px;border:0;background:transparent;font-size:26px;cursor:pointer}
    .rt-body{padding:16px;overflow:auto}
    .muted{color:#777}
    .sched-box{background:#f8fbff;border:1px solid #e6f1fb;border-radius:10px;padding:12px}
    .sched-box .day{display:flex;justify-content:space-between;padding:6px 10px;border-radius:8px}
    .sched-box .day.on{background:#eef7ff}
    .form-control, .form-select{border-radius:10px}
    .badge-day{display:inline-block;background:#eef7ff;color:#0d6efd;border-radius:20px;padding:2px 10px;margin:0 0 0 6px}
    .readonly-field{background:#f5f6f8 !important;pointer-events:none;color:#333}
  </style>
</head>
<body>
<div class="container-xxl">
  <div class="layout">
    ...
    <main class="main">
      <div class="app-header"><h1 class="m-0">تسجيل دخول مريض</h1></div>
      ...
      <section id="choose-disease" class="card-clean mb-3">
        ...
        <div class="disease-grid">
          <a class="disease-card js-open-book" data-spec="أمراض القلب"><i class="fa-solid fa-heart-pulse"></i><div>أمراض القلب</div></a>
          <a class="disease-card js-open-book" data-spec="الباطنية"><i class="fa-solid fa-staff-snake"></i><div>الباطنية</div></a>
          <a class="disease-card js-open-book" data-spec="العظام"><i class="fa-solid fa-bone"></i><div>العظام</div></a>
          <a class="disease-card js-open-book" data-spec="الأسنان"><i class="fa-solid fa-tooth"></i><div>الأسنان</div></a>
          <a class="disease-card js-open-book" data-spec="العيون"><i class="fa-regular fa-eye"></i><div>العيون</div></a>
          <a class="disease-card js-open-book" data-spec="الجلدية"><i class="fa-regular fa-face-smile"></i><div>الجلدية</div></a>
          <a class="disease-card js-open-book" data-spec="أطفال"><i class="fa-solid fa-baby"></i><div>أطفال</div></a>
          <a class="disease-card js-open-book" data-spec="نساء وولادة"><i class="fa-solid fa-person-pregnant"></i><div>نساء وولادة</div></a>
        </div>
      </section>
...
<div id="bookModal" class="rt-modal" aria-hidden="true">
  ...
  <div class="rt-dialog" role="dialog" aria-modal="true" aria-labelledby="bookTitle">
    ...
    <div class="rt-header"><h4 id="bookTitle" class="rt-title"><i class="fa-regular fa-calendar-plus"></i> حجز موعد جديد</h4></div>
...
(function($){
  ...
  const openModal = (prefSpec='')=>{
    ...
    loadSpecs().then(()=>{ if (prefSpec) $('#bf-spec').val(prefSpec).trigger('change'); });
  };
  const closeModal = ()=>{
  ...
  $(document).on('click','.js-open-book', function(e){ e.preventDefault(); openModal($(this).data('spec')||''); });
  ...
  function resetForm(){
    ...
    $('#bf-fees').val(''); $('#bf-spec-ro').val('');
    $('#bf-date').val(''); $('#bf-date-human').text('');
    $('#schedList').empty(); allowedDays=[];
  }

  // تحميل التخصصات
  let cachedSpecs=null;
  function loadSpecs(){
    if (cachedSpecs){ fillSpecs(cachedSpecs); return Promise.resolve(); }
    return $.getJSON('?ajax=specs').then(j=>{ if(j.ok){ cachedSpecs=j.data; fillSpecs(j.data); } });
  }
  ...
  $('#bf-spec').on('change', function(){
    ...
    $('#bf-day').prop('disabled', true).html('<option value="">— اختر اليوم —</option>');
    $('#bf-time').prop('disabled', true).html('<option value="">— اختر الوقت —</option>');
    $('#bf-fees').val(''); $('#bf-date').val(''); $('#bf-date-human').text('');
    $('#schedList').empty(); allowedDays=[];
    if(!spec) return;

    $.getJSON('?ajax=doctors_by_spec',{spec}).done(j=>{
      if(j.ok){
  ...
  $('#bf-doctor').on('change', function(){
    ...
    $.getJSON('?ajax=doctor_info',{id}).done(j=>{
      if(!j.ok) return;
  ...
  $('#bf-day').on('change', function(){
    ...
    $.getJSON('?ajax=slots', { id, date: dateStr }).done(s=>{
  ...
  function nextDateForDay(dow){
    ...
    const target = new Date(d); target.setDate(d.getDate() + diff);
  ...
  function renderSchedule(daysObj){
    ...
    if (!Object.keys(daysObj||{}).length){ $box.html('<div class="muted">لا يوجد دوام مُسجّل لهذا الطبيب.</div>'); return; }
    for(let i=0;i<7;i++){
      if (daysObj[i]){
