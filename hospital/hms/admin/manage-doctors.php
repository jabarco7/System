...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete') {
    if (!hash_equals($_SESSION['csrf_token'] ?? '', $_POST['csrf_token'] ?? '')) {
        $_SESSION['msg'] = 'خطأ في التحقق الأمني (CSRF). أعد المحاولة.';
...
function clean_spec($s)
{
    $s = preg_replace('/\badd-doctor\.php:\d+\s*/i', '', (string)$s);
    return trim($s, " -\t\n\r\0\x0B");
}
            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered align-middle">
                    ...
                    <tbody>
                            ...
                            <?php while ($row = $rows->fetch_assoc()): ?>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a href="manage-doctor-schedule.php?doctorId=<?= (int)$row['id'] ?>"
                                            class="action-btn"
                                            style="background:rgba(52,152,219,.12);color:#3498db;border:1px solid rgba(52,152,219,.3);margin-inline-start:6px;">
                                            <i class="fa-regular fa-calendar"></i> الدوام
                                        </a>
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="7" class="text-center text-muted py-4">لا توجد سجلات متاحة</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
