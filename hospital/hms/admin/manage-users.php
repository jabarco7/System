...
if (isset($_POST['ajax'])) {
    ...
    if ($_POST['ajax'] === 'get_user') {
        ...
        if (!$user) { echo json_encode(['ok'=>false,'msg'=>'المستخدم غير موجود']); exit; }
        echo json_encode(['ok'=>true,'user'=>$user], JSON_UNESCAPED_UNICODE); exit;
    }
    ...
    if ($_POST['ajax'] === 'update_user') {
        ...
        if ($id<=0 || $fullName==='' || $address==='' || $city==='' || $gender==='' || $email==='') {
            echo json_encode(['ok'=>false,'msg'=>'يرجى تعبئة جميع الحقول.']); exit;
        }
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            echo json_encode(['ok'=>false,'msg'=>'البريد الإلكتروني غير صالح.']); exit;
        }
        ...
        if (mysqli_fetch_assoc($duRes)) {
            echo json_encode(['ok'=>false,'msg'=>'البريد مستخدم بالفعل لمستخدم آخر.']); exit;
        }
        mysqli_stmt_close($du);

        // بناء استعلام التحديث
        if ($pwNew !== '' || $pwConf !== '') {
            if ($pwNew !== $pwConf) { echo json_encode(['ok'=>false,'msg'=>'كلمتا المرور غير متطابقتين.']); exit; }
            if (strlen($pwNew) < 6) { echo json_encode(['ok'=>false,'msg'=>'كلمة المرور يجب أن تكون 6 أحرف/أرقام على الأقل.']); exit; }
        ...
        if (mysqli_stmt_execute($up)) {
            echo json_encode(['ok'=>true,'msg'=>'تم الحفظ بنجاح']); exit;
        } else {
            echo json_encode(['ok'=>false,'msg'=>'تعذّر الحفظ.']); exit;
        }
    }
...
<head>
    ...
    <style>
        ...
        .table{width:100%;border-collapse:separate;min-width:900px;}
        ...
        .btn-delete:hover{background:rgba(231,76,60,.3);color:#fff;}
        .btn-edit{background:rgba(52,152,219,.15);color:#3498db;border:1px solid rgba(52,152,219,.3);}
        .btn-edit:hover{background:rgba(52,152,219,.3);color:#fff;}
                    ...
                    <tbody>
                        ...
                        if ($sql && mysqli_num_rows($sql) > 0):
                            while ($row = mysqli_fetch_assoc($sql)):
                                ...
                                $badgeClass = ($g === 'ذكر' || $g === 'male') ? 'badge-male' :
                                              (($g === 'انثى' || $g === 'female') ? 'badge-female' : '');
                        ?>
...
setTimeout(() => {
    ...
    if (alertEl) { alertEl.style.opacity = '0'; setTimeout(() => alertEl.remove(), 400); }
}, 4000);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  editModal = new bootstrap.Modal(modalEl, { backdrop: 'static' });
  ...
  document.querySelectorAll('.js-edit-user').forEach(btn=>{
    btn.addEventListener('click', function(e){
      ...
      fetch('manage-users.php', {
        ...
        body: new URLSearchParams({ajax:'get_user', id})
      })
      .then(r=>r.json())
      .then(data=>{
        if(!data || !data.ok){ showEditAlert(data?.msg || 'تعذّر جلب البيانات.', 'danger'); return; }
        const u = data.user;
        eu_id.value         = u.id;
  ...
  document.getElementById('saveUserBtn').addEventListener('click', function(){
    ...
    if (pw || pw2) {
      if (pw.length < 6) { showEditAlert('كلمة المرور يجب أن تكون 6 أحرف/أرقام على الأقل.', 'danger'); return; }
      if (pw !== pw2)   { showEditAlert('كلمتا المرور غير متطابقتين.', 'danger'); return; }
    }
