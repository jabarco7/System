...
/* -------------------------------------------
   1) جهّز هويات الربط: users.id و tblpatient.ID
-------------------------------------------- */
$userEmail = '';
if ($stmtU = $con->prepare("SELECT email FROM users WHERE id = ? LIMIT 1")) {
    $stmtU->bind_param("i", $uid);
    $stmtU->execute();
    $stmtU->bind_result($userEmail);
    $stmtU->fetch();
    $stmtU->close();
}

$patientId = -1; // قيمة لا تطابق شيء افتراضياً
if ($userEmail !== '' && $stmtP = $con->prepare("SELECT ID FROM tblpatient WHERE PatientEmail = ? LIMIT 1")) {
    $stmtP->bind_param("s", $userEmail);
    $stmtP->execute();
    $stmtP->bind_result($patientId);
    $stmtP->fetch();
    $stmtP->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cancel_id'])) {
    ...
    if ($aid > 0) {
        // نسمح بالإلغاء إذا كان userId يطابق users.id أو tblpatient.ID
        $sqlCancel = "
            UPDATE appointment
            SET userStatus = 0
            WHERE id = ?
        ...
        if ($stmtC = $con->prepare($sqlCancel)) {
            $stmtC->bind_param("iii", $aid, $uid, $patientId);
            $stmtC->execute();
            $affected = $stmtC->affected_rows;
            $stmtC->close();
...
/* عدّ الإجمالي */
$totalRows = 0;
$sqlCount = "SELECT COUNT(*) FROM appointment a WHERE a.userId = ? OR a.userId = ?";
if ($stmtCount = $con->prepare($sqlCount)) {
    $stmtCount->bind_param("ii", $uid, $patientId);
    $stmtCount->execute();
    $stmtCount->bind_result($totalRows);
    $stmtCount->fetch();
    $stmtCount->close();
} else {
    die("SQL COUNT error: " . $con->error);
}
$totalPages = max(1, (int)ceil($totalRows / $perPage));

/* -------------------------------------------
   4) جلب بيانات الصفحة الحالية
   - COALESCE للتخصص والرسوم (من appointment أو doctors)
   - تضمين LIMIT/OFFSET أرقامًا بعد تحويلها int
-------------------------------------------- */
$limit  = (int)$perPage;
$off    = (int)$offset;
...
$sql = "
    SELECT 
        ...
        COALESCE(a.consultancyFees, d.docFees, 0)          AS consultancyFees,
        a.appointmentDate,
        a.appointmentTime,
        a.postingDate,
        a.userStatus,
    ...
    WHERE a.userId = ? OR a.userId = ?
    ORDER BY a.id DESC
    LIMIT $limit OFFSET $off
";
if ($stmt = $con->prepare($sql)) {
    $stmt->bind_param("ii", $uid, $patientId);
    $stmt->execute();
    $res = $stmt->get_result();
    if ($res === false) {
    ...
    while ($r = $res->fetch_assoc()) {
        $rows[] = [
            ...
            'consultancyFees'      => $r['consultancyFees'],
            'appointmentDate'      => $r['appointmentDate'],
            'appointmentTime'      => $r['appointmentTime'],
            'postingDate'          => $r['postingDate'],
            'userStatus'           => (int)$r['userStatus'],
            'doctorStatus'         => (int)$r['doctorStatus'],
        ];
    }
    $stmt->close();
} else {
    die("SQL PREP error: " . $con->error);
}
?>
<!DOCTYPE html>
